...
#define MAX_PARALLEL_TESTS 30		/* size of an ascii uuid */
...
struct iperf_test
{
    ...
    struct iperf_master_test* mster_test;
...
{
    struct iperf_test *test_map[200];
//    int skt_to_test[1024];
    int currunt_conn_type;
    char      role;                             /* 'c' lient or 's' erver */
    enum iperf_mode mode;
    int       sender_has_retransmits;
    int       other_side_has_retransmits;       /* used if mode == BIDIRECTIONAL */
  ...
  //  void      (*reporter_callback) (struct iperf_test *);
    ...
    char      current_cookie[COOKIE_SIZE];
//    struct iperf_stream *streams;               /* pointer to list of struct stream */
//    SLIST_HEAD(slisthead, iperf_stream) streams;
    struct iperf_settings *settings;
